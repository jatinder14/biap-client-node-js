...
class OrderStatusService {
  ...
  async onOrderStatus(messageId) {
    try {
      ...
      console.log(
        "protocolOrderStatusResponse------------>",
        JSON.stringify(protocolOrderStatusResponse)
      );
          ...
          try {
            ...
            if (!onOrderStatusResponse.error) {
              ...
              if (dbResponse && dbResponse.length) {
                ...
                if (
                  orderSchema.state !==
                    onOrderStatusResponse?.message?.order?.state &&
                  onOrderStatusResponse?.message?.order?.state === "Completed"
                ) {
                  await sendEmail({
                    userEmail,
                    orderId,
                ...
                if (
                  onOrderStatusResponse?.message?.order?.state === "Cancelled"
                ) {
                ...
                orderSchema.fulfillments =
                  onOrderStatusResponse?.message?.order?.fulfillments;
                if (onOrderStatusResponse?.message?.order?.quote) {
                  ...
                  orderSchema.updatedQuote =
                    onOrderStatusResponse?.message?.order?.quote;
                }
                if (onOrderStatusResponse?.message?.order?.documents) {
                  // console.log("on status reponse qoute------->",onOrderStatusResponse?.message?.order?.quote)
                  orderSchema.documents =
                    onOrderStatusResponse?.message?.order?.documents;
                }

                let op = orderSchema?.items.map((e, i) => {
                  let temp =
                    onOrderStatusResponse.message?.order?.fulfillments?.find(
                      (fulfillment) => fulfillment?.id === e?.fulfillment_id
                    );
                  if (temp) {
                ...
                let protocolItems =
                  onOrderStatusResponse?.message?.order?.items;
                ...
                for (let fulfillment of onOrderStatusResponse.message?.order
                  ?.fulfillments) {
                  ...
                  if (!existingFulfillment) {
                    await FulfillmentHistory.create({
                      ...
                      state: fulfillment.state.descriptor.code,
                      updatedAt:
                        onOrderStatusResponse.message.order.updated_at.toString(),
                    });
                  }
                ...
                for (let item of protocolItems) {
                  ...
                  updatedItem = orderSchema.items.filter(
                  ...
                  let temp1 =
                    onOrderStatusResponse.message?.order?.fulfillments?.find(
                      (fulfillment) => fulfillment?.id === item?.fulfillment_id
                    );
  ...
  async updateForPaymentObject(orderRequest, protocolUpdateResponse) {
    try {
      ...
      const orderRequestDb = await getOrderRequest({
        transaction_id: orderRequest?.context?.transaction_id,
        requestType: "update",
      });
      ...
      if (!orderRequestDb?.request?.data?.payment) {
        ...
        console.log(
          "orderDetails?.updatedQuote?.price?.value----1->",
          orderDetails?.updatedQuote?.price?.value
        );
        ...
        console.log(
          "orderDetails?.updatedQuote?.price?.value----2->",
          protocolUpdateResponse.message.order.quote?.price?.value
        );
        console.log(
          "orderDetails?.updatedQuote?.price?.value---message id-->",
          protocolUpdateResponse.context.message_id
        );

        if (
          parseInt(updatedValue) >
          parseInt(protocolUpdateResponse.message.order.quote?.price?.value)
        ) {
          ...
          for (const item of updatedItems) {
            let updateItem = dbItems.find((i) => {
              return i.id === item.id;
            });
            if (updateItem) {
              console.log("update item found---->", updateItem.id);
              console.log(
                "update item found----item?.tags?.status>",
                item?.tags?.status
              );
              console.log(
                "update item found----updateItem.return_status",
                updateItem.return_status
              );
              //check the status
              if (
                ["Cancelled", "Liquidated", "Return_Picked"].includes(
                  item?.tags?.status
                ) &&
                item?.tags?.status !== updateItem.return_status
              ) {
                updateQoute = true;

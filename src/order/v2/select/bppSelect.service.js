      ...
      for (let [index, item] of cart.items.entries()) {
        ...
        if (item.customisations) {
          for (let customisation of item.customisations) {
            ...
            let tag = undefined;
            if (
              customisation.item_details?.tags &&
              customisation.item_details?.tags.length > 0
            ) {
              tag = customisation.item_details.tags.filter((i) => {
                return i.code === "type" || i.code === "parent";
              });
              let finalTags = [];
              for (let tg of tag) {
                tag;

...
class RazorPayController {
  ...
  async refundPayment(req, res, next) {
    ...
    try {
      ...
      return res.json({
        status: false,
        message: (e?.statusCode == 404) ? `paymnet with ${razorpayPaymentId} does not exists` : (e?.statusCode == 400) ? `${e?.error?.description}` : `We encountered an unexpected error while processing your request. Please try again later.`
      })
    }
  }
